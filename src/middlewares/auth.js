...
const verifyToken = (req, res, next) => {
  ...
  const checkBlacklistToken =
    "SELECT * FROM public.blacklist_token WHERE token = $1";
  db.query(checkBlacklistToken, [token], (err, res) => {
    if (err) return onFailed(res, 500, "Internal server error", err);
    if (res.rows.length > 0)
      onFailed(res, 401, "Invalid Token, You need to login first!", err);
    jwt.verify(token, SECRET_KEY, (err, decode) => {
      if (err)
        return onFailed(res, 401, "Invalid Token, You need to login first!");
      const { id, email, role, name } = decode;
      req.userInfo = {
        id,
        email,
        role,
