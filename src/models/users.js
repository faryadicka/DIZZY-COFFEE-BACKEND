...
const getAllUsersModel = (query) => {
  return new Promise((resolve, reject) => {
    ...
    const sql =
      "SELECT id, display_name, address, phone, image_profile, birthdate, gender, first_name, last_name, email FROM public.users LIMIT $1 OFFSET $2";
...
const getDetailUserModel = (id) => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT display_name, address, phone, image_profile, birthdate, gender, first_name, last_name, email FROM public.users WHERE id = $1";
  ...
  return new Promise((resolve, reject) => {
    ...
    db.query(sql, [newPassword, otp, email], (err, res) => {
      if (newPassword !== confirmPassword)
        return reject({
          message: "Set password doesn't match",
          status: 401,
          err,
        });
      if (err)
